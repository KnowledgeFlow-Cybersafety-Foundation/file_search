...
from ui.tabs import (
    render_overview_tab, render_search_tab, render_tag_cloud_tab,
    render_analytics_tab, render_summary_report_tab
)
...
def main():
    ...
    doc_folder, load_clicked, filters, search_method, max_results = (
        render_sidebar(searcher.documents)
    )
    ...
    if searcher.documents:
        ...
        with tab2:
            render_search_tab(
                filtered_docs, searcher, search_method, max_results
            )
        ...
        if 'original_docs' in locals():
            if len(filtered_docs) < len(original_docs):
                st.info(
                    ...
                    f"{len(original_docs)} documents (filtered)"
                )
