...
class ReportGenerator:
    ...
    def create_summary_report(
        ...
        report_data = {
            ...
            'summary_statistics': ReportGenerator._generate_summary_stats(
                documents
            )
        }
        ...
        if include_details:
            report_data['document_details'] = (
                ReportGenerator._generate_document_details(documents)
            )
        
        if include_analytics:
            report_data['analytics'] = ReportGenerator._generate_analytics(
                documents
            )
    ...
    def _generate_summary_stats(
        ...
        return {
            'total_word_count': sum(
                doc.get('word_count', 0) for doc in documents
            ),
            'average_readability': round(
                np.mean([doc.get('readability', 0) for doc in documents]), 1
            ),
            'total_size_kb': round(
                sum(doc.get('size_kb', 0) for doc in documents), 2
            ),
            'document_categories': dict(Counter([
                doc.get('business_category', 'Unknown') for doc in documents
            ])),
            'document_types': dict(Counter([
                doc.get('document_type', 'Unknown') for doc in documents
            ])),
            'urgency_levels': dict(Counter([
                doc.get('urgency_level', 'Unknown') for doc in documents
            ])),
    ...
    def _generate_analytics(
        ...
        complexity_scores = [
            doc.get('complexity_score', 0) for doc in documents
        ]
