...
from categorizer import DocumentCategorizer  # noqa: E402
...
class DocumentSearcher:
    ...
    def search(self, query: str, method: str = "keyword", top_k: int = 10) -> List[Dict[str, Any]]:
    ...
    def search_by_tags(self, selected_tags: List[str], top_k: int = 10) -> List[Dict[str, Any]]:
    ...
    def _keyword_search(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        for doc in self.documents:
            ...
            if score > 0:
                ...
                results.append({**doc, "score": score, "matches": matches, "context": context})

        return sorted(results, key=lambda x: x["score"], reverse=True)[:top_k]

    def _similarity_search(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        for i, doc in enumerate(self.documents):
            if similarities[i] > 0:
                ...
                results.append({**doc, "score": similarities[i], "context": context})

        return sorted(results, key=lambda x: x["score"], reverse=True)[:top_k]

    def _combined_search(self, query: str, top_k: int) -> List[Dict[str, Any]]:
    ...
    def _get_context(
        self, content: str, query_words: List[str], context_length: int = CONTEXT_LENGTH
    ) -> List[str]:
    ...
    def _categorize_documents(self) -> None:
        ...
        categorized_docs = []
        for doc in self.documents:
            analysis = self.categorizer.analyze_document(doc["content"], doc["filename"])
    ...
    def filter_documents(
        ...
        if category and category != "All":
            filtered = [doc for doc in filtered if doc.get("business_category") == category]

        if doc_type and doc_type != "All":
            filtered = [doc for doc in filtered if doc.get("document_type") == doc_type]

        if tags:
            for tag in tags:
                filtered = [doc for doc in filtered if tag in doc.get("tags", [])]

        if urgency and urgency != "All":
            filtered = [doc for doc in filtered if doc.get("urgency_level") == urgency]

        if min_readability is not None:
            filtered = [doc for doc in filtered if doc.get("readability", 0) >= min_readability]

        if max_readability is not None:
            filtered = [doc for doc in filtered if doc.get("readability", 100) <= max_readability]
    ...
    def generate_wordcloud_tags(
        ...
        # Generate word cloud
        wordcloud = WordCloud(**WORDCLOUD_CONFIG).generate_from_frequencies(tag_freq)
