...
from ui.components import (
    display_document_metrics, display_search_result,
    display_category_charts, display_tags_chart,
    display_analytics_charts, display_entities
)
from utils.helpers import (
    create_downloadable_summary, generate_filename_with_timestamp
)
from utils.constants import REPORT_FORMATS, MIME_TYPES
    ...
    if search_query:
        with st.spinner("Searching..."):
            if search_method == "tags":
                query_tags = [
                    tag.strip() for tag in search_query.split(',')
                ]
                results = searcher.search_by_tags(query_tags, max_results)
            else:
                results = searcher.search(
                    search_query, search_method, max_results
                )
...
def render_tag_cloud_tab(
    ...
    st.markdown(
        "Click on tags in the word cloud or select from the list "
        "to search documents!"
    )
    ...
    if wordcloud_data:
        ...
        with col1:
            ...
            for i, (tag, freq) in enumerate(top_tags):
                with cols[i % 5]:
                    ...
                    if st.button(
                        ...
                        if tag in st.session_state.selected_wordcloud_tags:
                            st.session_state.selected_wordcloud_tags.remove(
                                tag
                            )
                        else:
                            st.session_state.selected_wordcloud_tags.append(
                                tag
                            )
                        st.rerun()
            ...
            if st.session_state.selected_wordcloud_tags:
                st.write(
                    "**Selected Tags:**",
                    ", ".join(st.session_state.selected_wordcloud_tags)
                )
                ...
                if tag_results:
                    st.success(
                        f"Found {len(tag_results)} documents with "
                        "selected tags"
                    )
        ...
        with col2:
            ...
            st.markdown(
                f"• Documents with tags: "
                f"{len([d for d in documents if d.get('tags')])}"
            )
            avg_tags = (
                len([tag for doc in documents for tag in doc.get('tags', [])])
                / len(documents)
            )
...
def render_summary_report_tab(
    ...
    st.markdown(
        "Create a customized, downloadable summary of your document "
        "analysis!"
    )
    ...
    with col1:
        ...
        report_query = st.text_input(
            "Search Query (for report context)",
            placeholder=(
                "Optional: Enter the search query this report relates to"
            )
        )
    ...
    with col2:
        ...
        st.metric(
            "Total Words",
            f"{sum(doc.get('word_count', 0) for doc in documents):,}"
        )
        st.metric(
            "Categories",
            len(set([
                doc.get('business_category', 'Unknown')
    ...
    if st.button("📊 Generate Report", type="primary"):
        with st.spinner("Generating comprehensive report..."):
            ...
            if report_data:
                ...
                downloadable_content = create_downloadable_summary(
                ...
                if downloadable_content:
                    ...
                    if report_format == "json":
                        ...
                        st.text_area(
                            "Report Preview",
                            downloadable_content[:2000],
                            height=300
                        )
                    elif (report_format == "csv" and
                          'document_details' in report_data):
                        st.dataframe(
                            pd.DataFrame(
                    ...
                    filename = generate_filename_with_timestamp(
                        "document_summary", report_format
                    )
                    ...
                    st.error(
                        "Failed to generate downloadable content. Please "
                        "try a different format."
                    )
            else:
                st.error(
                    "Failed to generate report. Please ensure documents "
                    "are loaded."
                )
