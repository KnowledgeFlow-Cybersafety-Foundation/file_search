...
from utils.helpers import format_tags_display  # noqa: E402
...
def display_search_result(
    result: Dict[str, Any], index: int, search_query: str, search_method: str
) -> None:
    ...
    with st.expander(f"📄 {result['filename']} ({score_display})"):
        ...
        with col1:
            ...
            st.markdown(f"**🏢 Category:** " f"{result.get('business_category', 'Unknown')}")
            ...
            if result.get("sentiment"):
                ...
                st.markdown(
                    f"**😊 Sentiment:** {sentiment['label']} " f"({sentiment['polarity']:.2f})"
                )
            ...
            if "context" in result and result["context"]:
                ...
                for ctx in result["context"]:
                    ...
                    for word in search_query.split():
                        highlighted = re.sub(
                            f"({re.escape(word)})", r"**\1**", highlighted, flags=re.IGNORECASE
                        )
        ...
        with col2:
            ...
            if st.button("👁️ Preview Full Content", key=f"preview_{index}"):
                content_preview = result["content"][:2000]
                if len(result["content"]) > 2000:
                    content_preview += "..."
                st.text_area(
                    "Full Document Content", content_preview, height=300, key=f"content_{index}"
                )
...
def display_category_charts(documents: List[Dict[str, Any]]) -> None:
    ...
    with col1:
        if documents:
            ...
            fig = px.pie(
                category_counts, values="Count", names="Category", title="Business Categories"
            )
    ...
    with col2:
        if documents:
            ...
            fig = px.bar(type_counts, x="Type", y="Count", title="Document Types")
    ...
    if documents:
        ...
        if all_tags:
            ...
            top_tags = pd.DataFrame(
                {"Tag": list(tag_counts.keys())[:20], "Frequency": list(tag_counts.values())[:20]}
            )
            fig = px.bar(
                top_tags, x="Frequency", y="Tag", orientation="h", title="Most Common Tags"
            )
            st.plotly_chart(fig, use_container_width=True)
...
def display_analytics_charts(documents: List[Dict[str, Any]]) -> None:
    ...
    with col2:
        # Sentiment analysis
        sentiments = [doc.get("sentiment", {}).get("label", "Unknown") for doc in documents]
        ...
        fig = px.pie(
            sentiment_counts, values="Count", names="Sentiment", title="Sentiment Distribution"
        )
        st.plotly_chart(fig, use_container_width=True)
